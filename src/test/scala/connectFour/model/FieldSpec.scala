...
class FieldSpec extends flatspec.AnyFlatSpec with matchers.should.Matchers {
  ...
  it should "correctly put a stone in the matrix" in {
    val field = new Field(7, 6, Stone.Empty)
    val newField = field.put(new HumanPlayer(Stone.Red,"1"), 0,5)
    newField.matrix.cell(5, 0) shouldEqual Stone.Red
  }

  it should "return the correct column index to put the stone in" in {
    val field1 = new Field(7, 6, Stone.Empty)
    val newField1 = field1.put(new HumanPlayer(Stone.Red,"1"), 0,4)
    newField1.moveCorrect(0) shouldEqual 4
    newField1.put(new HumanPlayer(Stone.Yellow,"1"), 0,3).moveCorrect(0) shouldEqual 3
    newField1.put(new HumanPlayer(Stone.Red,"1"), 1,4).moveCorrect(0) shouldEqual 4
  }
  */
}
